...
RSpec.describe Esse::AsyncIndexing::Adapters::Sidekiq do
  ...
  describe ".push instance method", freeze_at: [2020, 7, 2, 12, 30, 50] do
    ...
    context "with a standard job" do
      it "adds a valid job hash to redis" do
        redis do |conn|
  ...
  describe ".coerce_to_worker class method", freeze_at: [2020, 7, 2, 12, 30, 50] do
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker")
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker", retry: false)
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker", retry: true)
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
          "retry" => false
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker", queue: "foo")
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker", queue: "bar")
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
          "queue" => "foo"
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker").with_args(1)
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker").with_args([1, 2])
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
          "args" => [1]
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker").with_job_jid("123")
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker").with_job_jid("456")
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
          "jid" => "123"
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker").created_at((Time.now - (MINUTE_IN_SECONDS * 10)).to_f)
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker").created_at((Time.now - (MINUTE_IN_SECONDS * 11)).to_f)
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker").enqueued_at((Time.now - (MINUTE_IN_SECONDS * 10)).to_f)
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker").enqueued_at((Time.now - (MINUTE_IN_SECONDS * 11)).to_f)
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
    ...
    specify do
      expected = Esse::AsyncIndexing::Worker.new("DummyWorker").at((Time.now + (MINUTE_IN_SECONDS * 10)).to_f)
      not_expected = Esse::AsyncIndexing::Worker.new("DummyWorker").at((Time.now + (MINUTE_IN_SECONDS * 11)).to_f)
      actual = described_class.coerce_to_worker(
        {
          "class" => "DummyWorker",
  ...
  def redis
    Esse.config.async_indexing.sidekiq.redis_pool.with do |conn|
      yield conn
    end
  end
end
